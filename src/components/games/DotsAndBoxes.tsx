...
export const DotsAndBoxes: React.FC<DotsAndBoxesProps> = ({
  ...
  const [gameState, setGameState] = useState<GameState>({
    horizontalLines: Array(4).fill(null).map(() => Array(5).fill(false)),
    verticalLines: Array(5).fill(null).map(() => Array(4).fill(false)),
  ...
  const handleLineClick = (type: 'horizontal' | 'vertical', row: number, col: number) => {
    ...
    if (boxesCompleted === 0) {
      newGameState.currentPlayer = newGameState.currentPlayer === 'player1' ? 'player2' : 'player1';
    }
    
    setGameState(newGameState);
    
    // Check for game over
    const totalBoxes = newGameState.scores.player1 + newGameState.scores.player2;
    if (totalBoxes === 16) {
      setGameOver(true);
      const winner = newGameState.scores.player1 > newGameState.scores.player2 ? 'player1' : 
                    newGameState.scores.player2 > newGameState.scores.player1 ? 'player2' : 'tie';
      setWinner(winner);
  ...
  const checkBoxWithState = (state: GameState, row: number, col: number): 'player1' | 'player2' | null => {
      ...
      <div className="bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-6 rounded-xl shadow-2xl border-4 border-yellow-400">
        <div className="text-white text-center mb-4">
          ...
          <div className="flex justify-center space-x-8 text-lg">
            <span className={`${playerColor === 'player1' ? 'text-blue-400' : 'text-red-400'}`}>
              Player 1: {gameState.scores.player1}
            </span>
            <span className={`${playerColor === 'player2' ? 'text-red-400' : 'text-blue-400'}`}>
              Player 2: {gameState.scores.player2}
            </span>
          </div>
          {!gameOver && (
            <p className="text-sm mt-2">
              Current Turn: {gameState.currentPlayer === playerColor ? 'Your turn' : "Opponent's turn"}
            </p>
          )}
          {gameOver && (
          ...
          {Array(9).fill(null).map((_, rowIndex) => 
            Array(9).fill(null).map((_, colIndex) => {
              ...
              if (isHorizontalLine) {
                ...
                const isDrawn = gameState.horizontalLines[lineRow]?.[lineCol];
                
                return (
                  <button
                    key={`${rowIndex}-${colIndex}`}
                    onClick={() => handleLineClick('horizontal', lineRow, lineCol)}
              ...
              if (isVerticalLine) {
                ...
                return (
                  <button
                    key={`${rowIndex}-${colIndex}`}
                    onClick={() => handleLineClick('vertical', lineRow, lineCol)}
