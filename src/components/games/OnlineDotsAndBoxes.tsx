...
export const OnlineDotsAndBoxes: React.FC<OnlineDotsAndBoxesProps> = ({
  ...
  const makeMove = async (
    ...
    const isMyTurn =
      (gameState.current_player === "player1" && isPlayer1) ||
    ...
    try {
        ...
        for (let c = 0; c < 4; c++) {
          if (!newBoxes[r][c]) {
            ...
            if (top && bottom && left && right) {
              newBoxes[r][c] = gameState.current_player as
                | "player1"
                | "player2";
      ...
      // Record the move
      await supabase.from("dots_and_boxes_moves").insert({
        game_id: gameId,
  ...
  return (
    <div className="flex flex-col items-center p-4 space-y-6">
      <div className="w-full max-w-4xl">
        ...
        <div className="bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-4 sm:p-6 rounded-xl shadow-2xl border-4 border-yellow-400 w-full max-w-full sm:max-w-2xl md:max-w-3xl lg:max-w-4xl mx-auto">
          <div className="text-white text-center mb-4">
            <h2 className="text-2xl font-bold mb-2">
              Dots & Boxes - Online Match
            </h2>

            <div className="grid grid-cols-2 gap-4 mb-4">
              <div
                className={`p-3 rounded-lg ${isPlayer1 ? "bg-blue-600" : "bg-slate-700"}`}
              >
              ...
              <div
                className={`p-3 rounded-lg ${isPlayer2 ? "bg-red-600" : "bg-slate-700"}`}
              >
                <div className="flex items-center justify-center gap-2">
                  <Users className="h-4 w-4" />
                  <span className="font-bold">
                    {gameState.player2?.username || "Waiting..."}
                  </span>
                </div>
            ...
            {gameState.game_status === "active" && (
              <p className="text-sm">
                {isMyTurn
                  ? "Your turn!"
                  : `${gameState.current_player === "player1" ? gameState.player1?.username : gameState.player2?.username}'s turn`}
              </p>
            )}
                ...
                Array(9)
                  ...
                  .map((_, colIndex) => {
                    ...
                    if (isHorizontalLine) {
                      ...
                      const isDrawn =
                        gameState.horizontal_lines[lineRow]?.[lineCol];
                    ...
                    if (isVerticalLine) {
                      ...
                      const lineCol = Math.floor(colIndex / 2);
                      const isDrawn =
                        gameState.vertical_lines[lineRow]?.[lineCol];
